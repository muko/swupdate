...
..
	...
	description in XML.
SWUpdate は、ライブラリ "libconfig" をイメージ記述のデフォルト パーサーとして使用します。
ただし、libconfig でサポートされているものとは異なる構文と言語に基づいて、SWUpdate を拡張し、独自のパーサーを追加することは可能です。
examples ディレクトリには、XML で記述された Lua で書かれたパーサーのコードがあります。
...
デフォルトのパーサーを使用して、sw-description は libconfig マニュアルで説明されている構文規則に従います。
基本型の説明については、http://www.hyperrealm.com/libconfig/libconfig_manual.html をご覧ください。
説明全体は、sw-description ファイル自体に含まれている必要があります。
#include ディレクティブの使用は、SWUpdate では許可されていません。


..
	The following example explains better the implemented tags:
次の例は、実装されたタグをよく説明しています。
...
最初のタグは "software" です。
説明全体がこのタグに含まれています。
`ボード固有の設定`_ を使用して、デバイスごとに設定をグループ化することができます。
...
この概念は、複数のデバイス用のリリースを含む単一のイメージを提供するように拡張できます。
各デバイスには独自のカーネル、dtb、およびルート ファイルシステムがあり、一部のパーツを共有することもできます。
...
現在、これは、ソフトウェアが実行されているデバイスを認識した後、どのイメージをインストールする必要があるかを確認する独自のパーサーを作成することによって管理されています (実際のプロジェクトで既に使用されています)。
...
..
	...
	appropriate image.
ソフトウェア コレクションと操作モードを使用して、デュアル コピー戦略を実装できます。
最も単純なケースは、ファームウェア イメージの 2 つのインストール場所を定義し、適切なイメージを選択して `SWUpdate` を呼び出すことです。
...
..
	...
	other slot.
このようにして、copy-1 を `/dev/mtd4` にインストールし、copy-2 を `/dev/mtd5` にインストールするように指定できます。
インストール場所を適切に選択することにより、 `SWUpdate` は他のスロットのファームウェアを更新します。
